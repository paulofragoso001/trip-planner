...
test.describe("mobile soft-launch UX", () => {
  ...
  test("home launch uses Almidy-owned globe before wallet actions", async ({ page }) => {
    ...
    await expect(page.getByTestId("almidy-launch-globe")).toHaveAttribute("data-hero-mode", "interactive-3d-globe");
    ...
    expect(heroVisual.mode).toBe("interactive-3d-globe");
  ...
  test("mobile home launch globe uses granted browser location for pin", async ({ page }) => {
    ...
    await expect(page.getByTestId("almidy-launch-google-globe")).toHaveCount(0);
    await expectNoHomeGoogleMapsScripts(page);
  });
  ...
  test("mobile home launch globe supports reduced motion and unknown country fallback", async ({ page }) => {
    await page.setViewportSize({ height: 900, width: 390 });
    await page.setExtraHTTPHeaders({ "x-cypress-dashboard": "true" });
    await page.emulateMedia({ reducedMotion: "reduce" });
    await page.setViewportSize({ height: 900, width: 390 });
