...
test("loads dashboard summary and route-split pages", async ({ page }) => {
  ...
  await expect(page.getByText("Trips database")).toBeVisible();
  await expect(page.getByText("Trip list")).toBeVisible();
  ...
  await expect(page.getByRole("heading", { exact: true, name: "Map" })).toBeVisible();
  await expect(page.getByTestId("connected-trip-map")).toBeVisible();
  await expect(page.getByText("Connected Google Maps route preview")).toBeVisible();
  await expect(page.getByRole("button", { name: "Share view" })).toBeVisible();

  await openDashboardRoute("/dashboard/trips/demo/budget");
  await expect(page.getByRole("heading", { exact: true, name: "Budget" })).toBeVisible();
  await expect(page.getByText("Budget alerts")).toBeVisible();

  await openDashboardRoute("/dashboard/trips/demo/sharing");
  await expect(page.getByRole("heading", { exact: true, name: "Sharing" })).toBeVisible();
  ...
  await expect(importsRoute.getByRole("heading", { exact: true, name: "Imports" })).toBeVisible();
  await expect(importsRoute.getByRole("heading", { exact: true, name: "Unfiled items" })).toBeVisible();
  await expect(importsRoute.getByRole("button", { name: "Connect" }).first()).toBeVisible();
