...
export const dashboardActionContracts = [
  ...
  },
  {
    ...
    target: { type: "route", value: "latest trip href or /dashboard/trips?view=list#new-trip" },
    requiredAuth: "dashboard-session",
  ...
  },
  {
    ...
    target: { type: "route", value: "/dashboard/trips?view=list#new-trip" },
    requiredAuth: "dashboard-session",
    requiredPermissions: ["signed-in-user"],
  ...
  },
  {
    ...
    label: "Import a Reservation Manually / Open Reservation Importer",
    kind: "navigation-only",
    surface: "Mobile dashboard launch manual reservation card",
    selectorHint: 'role=link[name=/Import a Reservation Manually|Open Reservation Importer/]',
    intendedFunction: "Open the existing manual imports/reservations workflow.",
    target: { type: "route", value: "/dashboard/imports" },
    requiredAuth: "dashboard-session",
    requiredPermissions: ["signed-in-user"],
    expectedSuccessUi: "Imports page opens with manual reservation import actions.",
    expectedFailureUi: "Dashboard auth boundary redirects unauthenticated users."
  },
  {
  ...
  },
  {
    ...
    selectorHint: 'role=button[name="Pro soon"]',
    intendedFunction: "Represent the unavailable membership trial without implying activation is implemented.",
    target: { type: "local-state", value: "disabled unavailable state" },
    requiredAuth: "dashboard-session",
    requiredPermissions: ["signed-in-user"],
    expectedSuccessUi: "The Pro action is disabled and labeled Pro soon.",
  ...
  {
    id: "reservation-importer-dismiss",
    label: "Dismiss reservation importer card",
    kind: "client-ui-state",
    surface: "Mobile reservation importer card",
    selectorHint: 'role=button[name="Dismiss reservation importer card"]',
    intendedFunction: "Dismiss the manual reservation importer prompt for the current view.",
    target: { type: "local-state", value: "showReservationImporter=false" },
    requiredAuth: "dashboard-session",
    requiredPermissions: ["signed-in-user"],
    expectedSuccessUi: "Reservation importer card disappears from the expanded trips sheet.",
    expectedFailureUi: "No network failure path; the card remains visible if React state cannot update."
  },
  {
    id: "settings-routed-row-actions",
    label: "Save your trips / Manual reservation importer / Trips Timeline / My Almidy Book / Need help? / Talk to us / Your Membership / About Almidy / Terms of Service / Privacy Policy",
    kind: "navigation-only",
    ...
    target: { type: "route", value: "/dashboard/account, /dashboard/imports, /dashboard/trips, /dashboard/profile/stats, mailto:support@almidy.app, /about, /terms, /privacy" },
    requiredAuth: "dashboard-session",
    requiredPermissions: ["signed-in-user"],
  ...
  },
  {
    ...
    label: "Custom Categories / Add Reservations via Email / Calendar Feed / Connect with Claude / MCP / Shortcuts / Currency / Distance Unit / Language / App Icon / Notifications / Widgets / Storage and Data / Review the App / App Updates / Share to a Friend",
    kind: "client-ui-state",
    surface: "Mobile settings rows",
