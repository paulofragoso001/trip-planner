...
export function ResetPasswordForm() {
  ...
  return <form className="mt-6 grid gap-4" onSubmit={submit}><label className="grid gap-2 text-sm font-bold">New password<AlmidyInput autoComplete="new-password" minLength={10} onChange={(event) => setPassword(event.target.value)} required type="password" value={password}/></label><label className="grid gap-2 text-sm font-bold">Confirm password<AlmidyInput autoComplete="new-password" minLength={10} onChange={(event) => setConfirmation(event.target.value)} required type="password" value={confirmation}/></label><button className="rounded-xl bg-slate-950 px-4 py-3 font-bold text-white disabled:opacity-50" disabled={pending} type="submit">{pending ? "Updating…" : "Update password"}</button><p aria-live="polite" className="text-sm text-slate-600">{message}</p></form>;
}
