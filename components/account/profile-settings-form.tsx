...
export function ProfileSettingsForm({ initialDisplayName }: { initialDisplayName: string }) {
  ...
  return <form className="rounded-[1.75rem] border border-almidy-border-subtle bg-white p-5 shadow-panel" onSubmit={submit}><p className="text-xs font-black uppercase tracking-[0.18em] text-orange-500">Profile</p><h2 className="mt-1 text-2xl font-black text-slate-950">Display name</h2><p className="mt-2 text-sm text-slate-600">This name is used consistently in your account, menu, and trip collaboration.</p><label className="mt-4 grid gap-2"><span className="text-sm font-bold">Name</span><input autoComplete="name" className="min-h-11 rounded-xl border border-almidy-border-subtle px-3" maxLength={80} minLength={2} onChange={(event) => setName(event.target.value)} required value={name}/></label><div className="mt-4 flex items-center gap-3"><button className="rounded-xl bg-slate-950 px-4 py-2 text-sm font-bold text-white disabled:opacity-50" disabled={saving} type="submit">{saving ? "Saving…" : "Save name"}</button><p aria-live="polite" className="text-xs text-slate-600">{status}</p></div></form>;
}
