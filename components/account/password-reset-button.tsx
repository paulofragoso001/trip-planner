...
export function PasswordResetButton() {
  ...
  return <section className="rounded-[1.75rem] border border-almidy-border-subtle bg-white p-5 shadow-panel"><p className="text-xs font-black uppercase tracking-[0.18em] text-orange-500">Security</p><h2 className="mt-1 text-2xl font-black text-slate-950">Change password</h2><p className="mt-2 text-sm text-slate-600">Request a secure reset link for your verified account email. Password completion happens on Almidy’s web flow.</p><button className="mt-4 rounded-xl bg-slate-950 px-4 py-2 text-sm font-bold text-white disabled:opacity-50" disabled={pending} onClick={requestReset} type="button">{pending ? "Requesting…" : "Email reset instructions"}</button><p aria-live="polite" className="mt-3 text-xs text-slate-600">{message}</p></section>;
}
